    ...
    <div className='grid grid-cols-[1.2fr_1fr_1fr_0.55fr_auto] gap-3 items-center group'>
      ...
      <div className='relative'>
        <input
          ...
          className={`w-full bg-slate-900 border rounded-lg px-3 py-2 text-sm placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
      ...
      <input
        type='number'
        min={0}
        value={row.vat === 0 ? '' : row.vat}
        onChange={(e) => { updateRow(row.id, 'vat', e.target.value); setResult(null); }}
        placeholder='%'
        title='VAT rate (%) at the time of this payment'
        className='bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500'
      />
